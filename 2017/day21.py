...
def part1(instructions: dict[tuple[str, ...], tuple[str, ...]]) -> int:
    ...
    for i in range(5):
        t = 2 + size % 2
        a = size // t
        pieces = (tuple((''.join(i) for i in l)) for l in sum((list(zip(*mit.chunked(l, a))) for l in mit.chunked(mit.chunked(pattern, t), size)), start=[]))
        new_pieces = list((instructions[p] for p in pieces))
        size += size // t
        pattern = ''.join((''.join(i) for l in mit.chunked(new_pieces, a) for i in zip(*l)))
