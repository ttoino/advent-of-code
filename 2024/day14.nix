...
let
  ...
  input = builtins.map (
    l:
    let
      a = builtins.map lib.strings.toInt (builtins.match "p=([0-9]+),([0-9]+) v=(-?[0-9]+),(-?[0-9]+)" l);
    in
    {
      pos = {
        x = builtins.elemAt a 0;
        y = builtins.elemAt a 1;
      };
      vel = {
        x = builtins.elemAt a 2;
        y = builtins.elemAt a 3;
      };
    }
