...
def part2(inp: int):
    ...
    for size in range(1, 31):
        ...
        for x, y in it.product(range(1, 302 - size), range(1, 302 - size)):
            if size == 1:
                d[x, y, size] = ((x + 10) * y + serial_number) * (x + 10) // 100 % 10 - 5
            else:
                d[x, y, size] = d[x, y, size - 1] + sum((d[x + dx, y + size - 1, 1] for dx in range(size))) + sum((d[x + size - 1, y + dy, 1] for dy in range(size))) - d[x + size - 1, y + size - 1, 1]
